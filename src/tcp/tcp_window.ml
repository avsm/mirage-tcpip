...
type t = {
  ...
  mutable ack_seq: Tcp_sequence.t;
  mutable ack_win: int;

  mutable snd_una: Tcp_sequence.t;
  mutable tx_nxt: Tcp_sequence.t;
  mutable rx_nxt: Tcp_sequence.t;
  mutable rx_nxt_inseq: Tcp_sequence.t;
  mutable fast_rec_th: Tcp_sequence.t;
  ...
  mutable rtt_timer_seq: Tcp_sequence.t;
...
let pp fmt t =
  Format.fprintf fmt
    ...
    Tcp_sequence.pp t.rx_nxt
    Tcp_sequence.pp t.rx_nxt_inseq
    Tcp_sequence.pp t.tx_nxt
    t.rx_wnd t.tx_wnd
    Tcp_sequence.pp t.snd_una
...
let t ~rx_wnd_scale ~tx_wnd_scale ~rx_wnd ~tx_wnd ~rx_isn ~tx_mss ~tx_isn =
  ...
  let rx_nxt_inseq = Tcp_sequence.incr rx_isn in
...
let valid t seq =
  ...
  let ledge = Tcp_sequence.(sub t.rx_nxt (of_int32 t.max_rx_wnd)) in
  let r = Tcp_sequence.between seq ledge redge in
  Log.debug (fun f -> f "sequence validation: seq=%a range=%a[%lu] res=%b"
    Tcp_sequence.pp seq Tcp_sequence.pp t.rx_nxt t.rx_wnd r);
  r

(* Advance received packet sequence number *)
let rx_advance t b =
  t.rx_nxt <- Tcp_sequence.add t.rx_nxt b

(* Early advance received packet sequence number for packet ordering *)
let rx_advance_inseq t b =
  t.rx_nxt_inseq <- Tcp_sequence.add t.rx_nxt_inseq b
...
let set_ack_seq_win t s w =
  MProf.Counter.increase count_ackd_segs (Tcp_sequence.(sub s t.ack_seq |> to_int));
  t.ack_seq <- s;
  t.ack_win <- w
...
module Make(Clock:V1.MCLOCK) = struct
  ...
  let tx_advance clock t b =
    ...
    t.tx_nxt <- Tcp_sequence.add t.tx_nxt b

  (* An ACK was received - use it to adjust cwnd *)
  let tx_ack clock t r win =
    set_tx_wnd t win;
    if t.fast_recovery then begin
      if Tcp_sequence.gt r t.snd_una then
        t.snd_una <- r;
      if Tcp_sequence.geq r t.fast_rec_th then begin
      ...
      if Tcp_sequence.gt r t.snd_una then begin
        t.backoff_count <- 0;
        t.snd_una <- r;
        if t.rtt_timer_on && Tcp_sequence.gt r t.rtt_timer_seq then begin
...
let tx_available t =
  let inflight = Tcp_sequence.to_int32 (Tcp_sequence.sub t.tx_nxt t.snd_una) in
...
let tx_totalbytes t =
  Tcp_sequence.(to_int (sub t.tx_nxt t.tx_isn))

let rx_totalbytes t =
  (-) Tcp_sequence.(to_int (sub t.rx_nxt t.rx_isn)) 1
