...
val t : rx_wnd_scale:int -> tx_wnd_scale:int -> rx_wnd:int ->
  tx_wnd:int -> rx_isn:Tcp_sequence.t -> tx_mss:int option -> tx_isn:Tcp_sequence.t -> t

val valid : t -> Tcp_sequence.t -> bool

val rx_advance : t -> Tcp_sequence.t -> unit
val rx_advance_inseq : t -> Tcp_sequence.t -> unit
val rx_nxt : t -> Tcp_sequence.t
val rx_nxt_inseq : t -> Tcp_sequence.t

module Make(C:V1.MCLOCK) : sig
  val tx_advance : C.t -> t -> Tcp_sequence.t -> unit
  val tx_ack: C.t -> t -> Tcp_sequence.t -> int -> unit
end
...
val ack_seq : t -> Tcp_sequence.t
val ack_win : t -> int

val set_ack_serviced : t -> bool -> unit
val set_ack_seq_win : t -> Tcp_sequence.t -> int -> unit
...
val alert_fast_rexmit : t -> Tcp_sequence.t -> unit
