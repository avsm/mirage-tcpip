...
struct
  ...
  module RXS = Tcp_segment.Rx(Time)
  module TXS = Tcp_segment.Tx(Time)(Clock)
  module ACK = Tcp_ack.Immediate
  module UTX = Tcp_user_buffer.Tx(Time)(Clock)
  module WIRE = Tcp_serialise.Make(Ip)
  module STATE = Tcp_state.Make(Time)

  type pcb = {
    id: WIRE.id;
    wnd: Tcp_window.t;            (* Tcp_window information *)
    ...
    urx: Tcp_user_buffer.Rx.t;    (* App rx buffer *)
  ...
  type t = {
    ...
    (* server connections the process of connecting - SYN-ACK sent
       waiting for ACK *)
    listens: (WIRE.id, (Tcp_sequence.t * ((pcb -> unit Lwt.t) * connection)))
        Hashtbl.t;
    (* clients in the process of connecting *)
    connects: (WIRE.id, (connection_result Lwt.u * Tcp_sequence.t)) Hashtbl.t;
  }

  let pp_pcb fmt pcb =
    Format.fprintf fmt "id=[%a] state=[%a]" WIRE.pp_id pcb.id Tcp_state.pp pcb.state
  ...
  module Tx = struct
    ...
    let xmit_pcb ip id ~flags ~wnd ~options ~seq (datav : Cstruct.t) =
      let window = Int32.to_int (Tcp_window.rx_wnd_unscaled wnd) in
      let rx_ack = Some (Tcp_window.rx_nxt wnd) in
      let syn = match flags with Tcp_segment.Syn -> true | _ -> false in
      let fin = match flags with Tcp_segment.Fin -> true | _ -> false in
      let rst = match flags with Tcp_segment.Rst -> true | _ -> false in
      let psh = match flags with Tcp_segment.Psh -> true | _ -> false in
    ...
    let send_rst { ip; _ } id ~sequence ~ack_number ~syn ~fin =
      ...
      let rx_ack = Some Tcp_sequence.(add sequence (of_int32 datalen)) in
    ...
    let close pcb =
      ...
      match Tcp_state.state pcb.state with
      | Tcp_state.Established | Tcp_state.Close_wait ->
        UTX.wait_for_flushed pcb.utx >>= fun () ->
        (let { wnd; _ } = pcb in
         STATE.tick pcb.state (Tcp_state.Send_fin (Tcp_window.tx_nxt wnd));
         TXS.output ~flags:Tcp_segment.Fin pcb.txq (Cstruct.create 0)
        )
      | _ ->
        Log.debug (fun fmt ->
            fmt "TX.close: close requested but no action needed, state=%a" Tcp_state.pp pcb.state);
        Lwt.return_unit
    ...
    let thread t pcb ~send_ack ~rx_ack  =
      ...
      let rec send_empty_ack () =
        ...
        let ack_number = Tcp_window.rx_nxt wnd in
        let flags = Tcp_segment.No_flags in
        let options = [] in
    ...
    let thread (pcb:pcb) ~rx_data =
      ...
      let rec rx_application_t () =
        ...
        let signal_ack = function
          ...
          | Some winadv when Tcp_sequence.(gt winadv zero) ->
              Tcp_window.rx_advance wnd winadv;
              ACK.receive ack (Tcp_window.rx_nxt wnd)
          | Some winadv ->
              Tcp_window.rx_advance wnd winadv;
              ACK.pushack ack (Tcp_window.rx_nxt wnd)
        in
        begin match data with
          | None ->
            (* don't send an ACK in this case; this already happened *)
            STATE.tick pcb.state Tcp_state.Recv_fin;
            Tcp_user_buffer.Rx.add_r urx None >>= fun () ->
            Lwt.return_unit
          | Some data ->
            ...
            let rec queue = function
              | []     -> Lwt.return_unit
              | hd::tl ->
                Tcp_user_buffer.Rx.add_r urx (Some hd) >>= fun () ->
                queue tl
            in
            queue data >>= fun _ ->
            rx_application_t ()
  ...
  module Wnd = struct
    ...
    let thread ~urx:_ ~utx ~wnd:_ ~state ~tx_wnd_update =
      ...
      let rec tx_window_t () =
        ...
        begin match Tcp_state.state state with
          | Tcp_state.Reset -> UTX.reset utx
  ...
  let resolve_wnd_scaling options rx_wnd_scaleoffer =
    let tx_wnd_scale = List.fold_left (fun a ->
        function Tcp_options.Window_size_shift m -> Some m | _ -> a
      ) None options in
    match tx_wnd_scale with
    | None -> (0, 0), []
    | Some tx_f ->
      (rx_wnd_scaleoffer, tx_f),
      (Tcp_options.Window_size_shift rx_wnd_scaleoffer :: [])

  type pcb_params =
    { tx_wnd: int;
      sequence: Tcp_sequence.t;
      options: Tcp_options.t list;
      tx_isn: Tcp_sequence.t;
      rx_wnd: int;
  ...
  let new_pcb t params id =
    ...
    let tx_mss = List.fold_left (fun a ->
        function Tcp_options.MSS m -> Some m | _ -> a
      ) None options
    in
    ...
    let wnd =
      Tcp_window.t ~rx_wnd_scale ~tx_wnd_scale ~rx_wnd ~tx_wnd ~rx_isn ~tx_mss
        ~tx_isn
    in
    ...
    let rx_buf_size = Tcp_window.rx_wnd wnd in
    let urx = Tcp_user_buffer.Rx.create ~max_size:rx_buf_size ~wnd in
    ...
    let state = Tcp_state.t ~on_close in
    let txq, _tx_t =
    ...
    (* Set up ACK module *)
    let ack = ACK.t ~send_ack ~last:(Tcp_sequence.incr rx_isn) in
  ...
  let new_server_connection t params id pushf =
    ...
    STATE.tick pcb.state Tcp_state.Passive_open;
    STATE.tick pcb.state (Tcp_state.Send_synack params.tx_isn);
    ...
    Tcp_stats.incr_listen ();
    (* Queue a SYN ACK for transmission *)
    let options = Tcp_options.MSS 1460 :: opts in
    TXS.output ~flags:Tcp_segment.Syn ~options pcb.txq (Cstruct.create 0) >>= fun () ->
    Lwt.return (pcb, th)
    ...
    if ack then
        ...
        | None ->
          ...
          | Some (_, (_, (pcb, th))) ->
            Hashtbl.remove t.listens id;
            Tcp_stats.decr_listen ();
            STATE.tick pcb.state Tcp_state.Recv_rst;
            Lwt.cancel th;
            Lwt.return_unit
          | None ->
  ...
  let process_syn t id ~listeners ~tx_wnd ~ack_number ~sequence ~options ~syn ~fin =
    ...
    | Some pushf ->
      let tx_isn = Tcp_sequence.of_int ((Random.int 65535) + 0x1AFE0000) in
  ...
  let rec writefn pcb wfn data =
    ...
    | Tcp_state.Established | Tcp_state.Close_wait -> begin
  ...
  let connect t ~dst ~dst_port =
    ...
    let tx_isn = Tcp_sequence.of_int ((Random.int 65535) + 0x1BCD0000) in
    (* TODO: This is hardcoded for now - make it configurable *)
    let rx_wnd_scaleoffer = wscale_default in
    let options =
      Tcp_options.MSS 1460 :: Tcp_options.Window_size_shift rx_wnd_scaleoffer :: []
    in
    let window = 5840 in
