...
module Rx = struct
 ...
 (* TODO: check that flow control works on the rx side - ie if the application
  ...
  let notify_size_watcher t =
    ...
    Tcp_window.set_rx_wnd t.wnd rx_wnd;
...
module Tx(Time:V1_LWT.TIME)(Clock:V1.MCLOCK) = struct
  ...
  let available t =
    ...
    match a < (Int32.of_int (Tcp_window.tx_mss t.wnd)) with
    | true -> 0l
    | false -> a

  (* Check how many bytes are available to write to wire *)
  let available_cwnd t =
    Tcp_window.tx_available t.wnd
  ...
  let rec clear_buffer t =
    ...
    in
    let get_pkt_to_send () =
      let avail_len = min (available_cwnd t) (Int32.of_int (Tcp_window.tx_mss t.wnd)) in
    ...
    | false ->
      ...
      | Some pkt ->
        ...
        TXS.output ~flags:Tcp_segment.Psh t.txq b >>= fun () ->
        clear_buffer t
  ...
  let transmit_segments ~mss ~txq datav =
    let transmit acc =
      ...
      TXS.output ~flags:Tcp_segment.Psh txq b
    in
  ...
  let write t datav =
    let l = lenv datav in
    let mss = Int32.of_int (Tcp_window.tx_mss t.wnd) in
    match Lwt_sequence.is_empty t.buffer &&
          (l = mss || not (Tcp_window.tx_inflight t.wnd)) with
    | false ->
    ...
    | true ->
      ...
      | true ->
        ...
        Lwt.return_unit
      | false ->
        let max_size = Tcp_window.tx_mss t.wnd in
  ...
  let reset t =
    (* FIXME: duplicated code with Tcp_segment.reset_seq *)
