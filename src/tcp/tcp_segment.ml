...
module Rx(Time:V1_LWT.TIME) = struct
  ...
  let pp_segment fmt {header; payload} =
    Format.fprintf fmt
      ...
      Tcp_sequence.pp header.sequence Tcp_sequence.pp header.ack_number
      header.ack header.rst header.syn header.fin
      header.window (Cstruct.len payload)

  let len seg =
    Tcp_sequence.of_int ((Cstruct.len seg.payload) +
  ...
  module S = Set.Make(struct
      type t = segment
      let compare a b = (Tcp_sequence.compare a.header.sequence b.header.sequence)
    end)

  type t = {
    mutable segs: S.t;
    rx_data: (Cstruct.t list option * Tcp_sequence.t option) Lwt_mvar.t; (* User receive channel *)
    tx_ack: (Tcp_sequence.t * int) Lwt_mvar.t; (* Acks of our transmitted segs *)
    wnd: Tcp_window.t;
    state: Tcp_state.t;
  }
  ...
  let pp fmt t =
    let pp_v fmt seg =
      Format.fprintf fmt "%a[%a]" Tcp_sequence.pp seg.header.sequence Tcp_sequence.pp (len seg)
    in
  ...
  let check_valid_segment q seg =
    if seg.header.rst then
      if Tcp_sequence.compare seg.header.sequence (Tcp_window.rx_nxt q.wnd) = 0 then
        `Reset
      else if Tcp_window.valid q.wnd seg.header.sequence then
        `ChallengeAck
      else
        `Drop
      ...
      let min = Tcp_sequence.(sub (Tcp_window.tx_una q.wnd) (of_int32 (Tcp_window.max_tx_wnd q.wnd))) in
      if Tcp_sequence.between seg.header.ack_number min (Tcp_window.tx_nxt q.wnd) then
        `Ok
      else
        (* rfc5961 5.2 *)
        `ChallengeAck
  ...
  let send_challenge_ack q =
    ...
    Lwt_mvar.put q.rx_data (Some [], Some Tcp_sequence.zero)
  ...
  let input (q:t) seg =
    ...
    | `Ok ->
      ...
      let ready, waiting = S.fold (fun seg acc ->
          match Tcp_sequence.compare seg.header.sequence (Tcp_window.rx_nxt_inseq q.wnd) with
          | (-1) ->
            (* Tcp_sequence number is in the past, probably an overlapping
          ...
          | 0 ->
            ...
            Tcp_window.rx_advance_inseq q.wnd (len seg);
            (S.add seg ready), waiting
          | 1 ->
            (* Tcp_sequence is in the future, so can't use it yet *)
            force_ack := true;
            let (ready,waiting) = acc in
      ...
      let tx_ack =
        if seg.header.ack && (Tcp_sequence.geq seg.header.ack_number (Tcp_window.ack_seq q.wnd)) then begin
          TS.tick q.state (Tcp_state.Recv_ack seg.header.ack_number);
          let data_in_flight = Tcp_window.tx_inflight q.wnd in
          let ack_has_advanced = (Tcp_window.ack_seq q.wnd) <> seg.header.ack_number in
          let win_has_changed = (Tcp_window.ack_win q.wnd) <> seg.header.window in
          if ((data_in_flight && (Tcp_window.ack_serviced q.wnd || not ack_has_advanced)) ||
              (not data_in_flight && win_has_changed)) then begin
            Tcp_window.set_ack_serviced q.wnd false;
            Tcp_window.set_ack_seq_win q.wnd seg.header.ack_number seg.header.window;
            Lwt_mvar.put q.tx_ack ((Tcp_window.ack_seq q.wnd), (Tcp_window.ack_win q.wnd))
          end else begin
            Tcp_window.set_ack_seq_win q.wnd seg.header.ack_number seg.header.window;
            Lwt.return_unit
          end
      ...
      let urx_inform =
        ...
        let elems_r, winadv = S.fold (fun seg (acc_l, acc_w) ->
            ...
            (Tcp_sequence.add (len seg) acc_w)
          ) ready ([], Tcp_sequence.zero) in
        let elems = List.rev elems_r in
        let w = if !force_ack || Tcp_sequence.(gt winadv zero)
        ...
        (if fin ready then begin
            ...
            Lwt_mvar.put q.rx_data (None, Some Tcp_sequence.zero)
          end else Lwt.return_unit)
      in
      tx_ack <&> urx_inform
    | `ChallengeAck ->
    ...
    | `Drop ->
      Lwt.return_unit
    | `Reset ->
      TS.tick q.state Tcp_state.Recv_rst;
      (* Abandon our current segments *)
      q.segs <- S.empty;
      (* Signal TX side *)
      let txalert ack_svcd =
        if not ack_svcd then Lwt.return_unit
        else Lwt_mvar.put q.tx_ack (Tcp_window.ack_seq q.wnd, Tcp_window.ack_win q.wnd)
      in
      txalert (Tcp_window.ack_serviced q.wnd) >>= fun () ->
...
module Tx (Time:V1_LWT.TIME) (Clock:V1.MCLOCK) = struct
  ...
  module TS = Tcp_state.Make(Time)
  module TT = Tcp_timer.Make(Time)
  module TX = Tcp_window.Make(Clock)

  type xmit = flags:tx_flags -> wnd:Tcp_window.t -> options:Tcp_options.t list ->
    seq:Tcp_sequence.t -> Cstruct.t -> unit Lwt.t
  ...
  (* Tcp_sequence length of the segment *)
  let len seg =
    Tcp_sequence.of_int
  ...
  type t = {
    ...
    wnd: Tcp_window.t;                 (* TCP Tcp_window information *)
    state: Tcp_state.t;                (* state of the TCP connection associated
                                      with this queue *)
    tx_wnd_update: int Lwt_mvar.t; (* Received updates to the transmit window *)
    rexmit_timer: Tcp_timer.t;      (* Retransmission timer for this connection *)
  ...
  let ontimer xmit st segs wnd seq =
    match Tcp_state.state st with
    | Tcp_state.Syn_rcvd _ | Tcp_state.Established | Tcp_state.Fin_wait_1 _
    | Tcp_state.Close_wait | Tcp_state.Last_ack _ ->
      begin match peek_opt_l segs with
        | None -> Lwt.return Tcp_timer.Stoptimer
        | Some rexmit_seg ->
          match rexmit_seg.seq = seq with
          | false ->
            Log.debug (fun fmt ->
                fmt "PUSHING TIMER - new time=%Lu, new seq=%a"
                  (Tcp_window.rto wnd) Tcp_sequence.pp rexmit_seg.seq);
            let ret =
              Tcp_timer.ContinueSetPeriod (Tcp_window.rto wnd, rexmit_seg.seq)
            in
            Lwt.return ret
          | true ->
            if (Tcp_window.max_rexmits_done wnd) then (
              (* TODO - include more in log msg like ipaddrs *)
              Log.debug (fun f -> f "Max retransmits reached: %a" Tcp_window.pp wnd);
              Log.info (fun fmt -> fmt "Max retransmits reached for connection - terminating");
              TS.tick st Tcp_state.Timeout;
              Lwt.return Tcp_timer.Stoptimer
            ) else (
              let flags = rexmit_seg.flags in
              let options = [] in (* TODO: put the right options *)
              Log.debug (fun fmt ->
                  fmt "TCP retransmission triggered by timer! seq = %d"
                    (Tcp_sequence.to_int rexmit_seg.seq));
              Lwt.async
                (fun () -> xmit ~flags ~wnd ~options ~seq rexmit_seg.data);
              Tcp_window.alert_fast_rexmit wnd rexmit_seg.seq;
              Tcp_window.backoff_rto wnd;
              Log.debug (fun fmt -> fmt "Backed off! %a" Tcp_window.pp wnd);
              Log.debug (fun fmt ->
                  fmt "PUSHING TIMER - new time = %Lu, new seq = %a"
                    (Tcp_window.rto wnd) Tcp_sequence.pp rexmit_seg.seq);
              let ret =
    ...
    | _ -> Lwt.return Tcp_timer.Stoptimer

  let rec clearsegs q ack_remaining segs =
    match Tcp_sequence.(gt ack_remaining zero) with
    | false -> Tcp_sequence.zero (* here we return 0l instead of ack_remaining in case
    ...
    | true ->
      ...
      | Some s ->
        ...
        match Tcp_sequence.lt ack_remaining seg_len with
        | true ->
        ...
        | false ->
          ack_segment q s;
          clearsegs q (Tcp_sequence.sub ack_remaining seg_len) segs
  ...
  let rto_t q tx_ack =
    ...
    let rec tx_ack_t () =
      let serviceack dupack ack_len seq win =
        ...
        TX.tx_ack q.clock q.wnd (Tcp_sequence.sub seq partleft) win;
        match dupack || Tcp_window.fast_rec q.wnd with
        | true ->
          q.dup_acks <- q.dup_acks + 1;
          if q.dup_acks = 3 ||
            (Tcp_sequence.to_int32 ack_len > 0l) then begin
            (* alert window module to fall into fast recovery *)
            Tcp_window.alert_fast_rexmit q.wnd seq;
            ...
            Log.debug (fun fmt ->
                fmt "TCP fast retransmission seq=%a, dupack=%a"
                  Tcp_sequence.pp rexmit_seg.seq Tcp_sequence.pp seq);
            let { wnd; _ } = q in
            let flags=rexmit_seg.flags in
            let options=[] in (* TODO: put the right options *)
            Lwt.async
      ...
      Tcp_window.set_ack_serviced q.wnd true;
      let seq = Tcp_window.ack_seq q.wnd in
      let win = Tcp_window.ack_win q.wnd in
      begin match Tcp_state.state q.state with
        | Tcp_state.Reset ->
          ...
          Lwt.return_unit
        | _ ->
          let ack_len = Tcp_sequence.sub seq (Tcp_window.tx_una q.wnd) in
          let dupacktest () =
            0l = Tcp_sequence.to_int32 ack_len &&
            Tcp_window.tx_wnd_unscaled q.wnd = Int32.of_int win &&
  ...
  let create ~clock ~xmit ~wnd ~state ~rx_ack ~tx_ack ~tx_wnd_update =
    ...
    let period_ns = Tcp_window.rto wnd in
  ...
  *)
  let output ?(flags=No_flags) ?(options=[]) q data =
    ...
    let { wnd; _ } = q in
    let ack = Tcp_window.rx_nxt wnd in
    ...
    let q_rexmit () =
      match Tcp_sequence.(gt seq_len zero) with
      | false -> Lwt.return_unit
      | true ->
        lwt_sequence_add_r seg q.segs;
        let p = Tcp_window.rto q.wnd in
