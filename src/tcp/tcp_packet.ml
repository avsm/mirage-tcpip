...
let pp fmt t =
  Format.fprintf fmt
    ...
    Tcp_sequence.pp t.sequence Tcp_sequence.pp t.ack_number
    t.ack t.rst t.syn t.fin t.window Tcp_options.pps t.options
  ...
  let of_cstruct pkt =
    ...
    let options data_offset pkt =
      if data_offset > 20 then
        Tcp_options.unmarshal (Cstruct.shift pkt sizeof_tcp)
    ...
    let sequence = get_tcp_sequence pkt |> Tcp_sequence.of_int32 in
    let ack_number = get_tcp_ack_number pkt |> Tcp_sequence.of_int32 in
...
module Marshal = struct
  ...
  let unsafe_fill ~pseudoheader ~payload t buf options_len =
    ...
    set_tcp_sequence buf (Tcp_sequence.to_int32 t.sequence);
    set_tcp_ack_number buf (Tcp_sequence.to_int32 t.ack_number);
  ...
  let into_cstruct ~pseudoheader ~payload t buf =
    ...
    let insert_options options_frame =
      ...
      |options ->
        try
          Ok (Tcp_options.marshal options_frame options)
        with
  ...
  let make_cstruct ~pseudoheader ~payload t =
    let options_buf = Cstruct.create 40 in (* more than 40 bytes of options can't
                                              ...
                                              the tcp header *)
    let options_len = Tcp_options.marshal options_buf t.options in
