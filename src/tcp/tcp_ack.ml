...
module type M = sig
  ...
  (* called when new data is received *)
  val pushack: t -> Tcp_sequence.t -> unit Lwt.t
...
module Immediate : M = struct
  ...
  type t = {
    mutable send_ack: Tcp_sequence.t Lwt_mvar.t;
...
module Delayed (Time:V1_LWT.TIME) : M = struct
  ...
  type delayed_r = {
    ...
    mutable delayedack: Tcp_sequence.t;
  ...
  let ontimer r s  =
    ...
    | false -> Lwt.return Tcp_timer.Stoptimer
    | true  ->
      match r.delayedack = s with
      | false ->
        Lwt.return (Tcp_timer.Continue r.delayedack)
      | true ->
        r.delayed <- false;
        transmitack r s >>= fun () ->
