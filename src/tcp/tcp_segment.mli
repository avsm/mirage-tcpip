...
module Rx (T:V1_LWT.TIME) : sig
  ...
  val create:
    rx_data:(Cstruct.t list option * Tcp_sequence.t option) Lwt_mvar.t ->
    wnd:Tcp_window.t ->
    state:Tcp_state.t ->
    tx_ack:(Tcp_sequence.t * int) Lwt_mvar.t ->
    t
...
module Tx (Time:V1_LWT.TIME)(Clock:V1.MCLOCK) : sig
  ...
  val create:
    clock:Clock.t -> xmit:xmit -> wnd:Tcp_window.t -> state:Tcp_state.t ->
    rx_ack:Tcp_sequence.t Lwt_mvar.t ->
    tx_ack:(Tcp_sequence.t * int) Lwt_mvar.t ->
    tx_wnd_update:int Lwt_mvar.t -> t * unit Lwt.t

  val output:
    ?flags:tx_flags -> ?options:Tcp_options.t list -> t -> Cstruct.t -> unit Lwt.t
