...
let pp_action fmt = function
  ...
  | Recv_synack x -> pf fmt "Recv_synack(%a)" Tcp_sequence.pp x
  | Recv_ack x    -> pf fmt "Recv_ack(%a)" Tcp_sequence.pp x
  | Recv_fin      -> pf fmt "Recv_fin"
  (*  | Recv_finack x -> pf fmt "Recv_finack(%a)" Tcp_sequence.pp x *)
  | Send_syn x    -> pf fmt "Send_syn(%a)" Tcp_sequence.pp x
  | Send_synack x -> pf fmt "Send_synack(%a)" Tcp_sequence.pp x
  | Send_rst      -> pf fmt "Send_rst"
  | Send_fin x    -> pf fmt "Send_fin(%a)" Tcp_sequence.pp x
  | Timeout       -> pf fmt "Timeout"

let pp_tcpstate fmt = function
  | Closed       -> pf fmt "Closed"
  | Listen       -> pf fmt "Listen"
  | Syn_rcvd x   -> pf fmt "Syn_rcvd(%a)" Tcp_sequence.pp x
  | Syn_sent x   -> pf fmt "Syn_sent(%a)" Tcp_sequence.pp x
  | Established  -> pf fmt "Established"
  | Close_wait   -> pf fmt "Close_wait"
  | Last_ack x   -> pf fmt "Last_ack(%a)" Tcp_sequence.pp x
  | Fin_wait_1 x -> pf fmt "Fin_wait_1(%a)" Tcp_sequence.pp x
  | Fin_wait_2 i -> pf fmt "Fin_wait_2(%d)" i
  | Closing x    -> pf fmt "Closing(%a)" Tcp_sequence.pp x
...
module Make(Time:V1_LWT.TIME) = struct
  ...
  let tick t (i:action) =
    let diffone x y = Tcp_sequence.incr y = x in
    let tstr s (i:action) =
      match s, i with
